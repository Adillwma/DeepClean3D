...
noise_points = 10000
signal_points = 2000 #2000 #30 #200
time_dimension = 1000
...
output_name = f'{signal_points} scan with {noise_points} noise points. keep_photons={keep_photons}'
...
# Calculate the interval in milliseconds based on the frame rate
interval = 1000 / frame_rate


"""
# STANDARD SINGLE ANAIMATIONS
...
def update(frame):
    ...
    if keep_photons:
        ax.imshow(np.where((data_values <= current_time), data_values, 0), cmap=cmap)
    else:
        ax.imshow(np.where((data_values == current_time), data_values, 0), cmap=cmap)
    ax.set_title(f'MCP Scan \nCurrent Time: {current_time * 50 /1000:.2f} ns')
    ax.axis('off')
...
anim.save(f'{output_dir}{output_name}.gif', writer='imagemagick', fps=frame_rate)

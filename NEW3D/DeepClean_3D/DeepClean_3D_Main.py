...
num_epochs = 100   #40                                            #User controll to set number of epochs (Hyperparameter)
...
data_path = r"C:\Users\Student\Documents\UNI\Yr 3\DeepClean Repo Backup\DeepClean-Noise-Suppression-for-LHC-B-Torch-Detector 01\Datasets\\"    #"C:/Users/Student/Desktop/fake im data/"  #"/local/path/to/the/images/"
...
plot_every_other = 5       #(Default = 1) //MUST BE INTEGER INPUT//  #If debug loader batch is enabled this sets the interval for printing for user, 1 is every single img in the batch, 2 is every other img, 5 is every 5th image etc 
...
class ACBLoss(torch.nn.Module):
    def __init__(self, zero_weighting=5, nonzero_weighting=1):
        """
...
loss_fn = ACBLoss() #torch.nn.MSELoss()
...
try:     
    for epoch in range(num_epochs):                              #For loop that iterates over the number of epochs where 'epoch' takes the values (0) to (num_epochs - 1)
        ...
        if epoch % plot_every_other == 0:
            plot_ae_outputs_den(encoder,decoder,epoch + 1, noise_factor=noise_factor)

            #%% - Save Model
            #join encoder and decoder into one model
            model = nn.Sequential(encoder, decoder)
            torch.save(model.state_dict(), r'C:\Github\DeepClean-Noise-Suppression-for-LHC-B-Torch-Detector\NEW3D\model2.pth')

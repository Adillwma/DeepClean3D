from simple_cross.Comb_X_Gen import simp_generator
...
# Gen Signals  - can clean up all the intro code to one line if sort out args slightly (needs number of images removed and then no ratios to types of images just straiht values, and finally the shift and rotate params dshould accept boolean instead of 0-1 for user ease of following code) ALSO NEEDS A OUTPUT_DIR ARG
...
# Generate X signals
simp_generator(datasplit_ratios, total_x_images, sig_pts, xdim, ydim, time_dimension, shift)    #needs a output dir arg

...
###3D plotter
def generatexyz(pixel_block_3d): 
    hits_3d = np.argwhere(pixel_block_3d.squeeze() >= reconstruction_threshold)    
    noh = (np.shape(hits_3d)[0])
    x3d = hits_3d[:,2]
    y3d = hits_3d[:,1]
    z3d = hits_3d[:,0]
    return (x3d, y3d, z3d, noh)

def plot_3D(original_pixel_block_3d, reconstructed_pixel_block_3d):
    x1,y1,z1, noh1 = generatexyz(original_pixel_block_3d)
    x2, y2, z2, noh2 = generatexyz(reconstructed_pixel_block_3d)
    fig = plt.figure()
    ax1 = fig.add_subplot(211, projection='3d')
    ax2 = fig.add_subplot(212, projection='3d')
    ax1.scatter(x1,y1,z1)  
    ax2.scatter(x2,y2,z2)  
    Out_Label =  graphics_dir + f'{model_save_name} - 3D Comparison - Epoch {epoch}.png'
...
def plot_ae_outputs_den(encoder, decoder, epoch, model_save_name, time_dimension, reconstruction_threshold, n=10, noise_factor=0.3):       #Defines a function for plotting the output of the autoencoder. And also the input + clean training data? Function takes inputs, 'encoder' and 'decoder' which are expected to be classes (defining the encode and decode nets), 'n' which is the number of ?????Images in the batch????, and 'noise_factor' which is a multiplier for the magnitude of the added noise allowing it to be scaled in intensity.  
    ...
    fig = plt.figure(figsize=(16,4.5))                                      #Sets the figure size
    ...
    for i in range(n):                                                #Runs for loop where 'i' itterates over 'n' total values which range from 0 to n-1
      ...
      ax = fig.add_subplot(3, n, i+1, projection='3d')                                    #Creates a number of subplots for the 'Original images??????' i.e the labels. the position of the subplot is i+1 as it falls in the first row
      ...
      raw_data_out = img.cpu().squeeze().numpy()
      y, x, z, _ = generatexyz(raw_data_out)
      ax.scatter(y, x, z)
      ...
      ax = fig.add_subplot(3, n, i + 1 + n, projection='3d')                                 #Creates a number of subplots for the 'Corrupted images??????' i.e the labels. the position of the subplot is i+1+n as it falls in the second row      
      imn_data_out = image_noisy.cpu().squeeze().numpy()
      y2, x2, z2, _ = generatexyz(imn_data_out)
      ax.scatter(y2, x2, z2)  
      ax.get_xaxis().set_visible(False)                                   #Hides the x axis from showing in the plot as we are plotting images not graphs (we may want to retain axis?)
      ax.get_yaxis().set_visible(False)                                   #Hides the y axis from showing in the plot as we are plotting images not graphs (we may want to retain axis?)
      if i == n//2:                                                       #n//2 divides n by 2 without any remainder, i.e 6//2=3 and 7//2=3. So this line checks to see if i is equal to half of n without remainder. it will be yes once in the loop. not sure of its use
      ...
      ax = fig.add_subplot(3, n, i + 1 + n + n, projection='3d')                            #Creates a number of subplots for the 'Reconstructed images??????' i.e the labels. the position of the subplot is i+1+n+n as it falls in the third row
      rec_data_out = rec_img.cpu().squeeze().numpy()
      y3, x3, z3, _ = generatexyz(rec_data_out)
      ax.scatter(y3, x3, z3)
      ax.get_xaxis().set_visible(False)                                   #Hides the x axis from showing in the plot as we are plotting images not graphs (we may want to retain axis?)
      ax.get_yaxis().set_visible(False)                                   #Hides the y axis from showing in the plot as we are plotting images not graphs (we may want to retain axis?)
      if i == n//2:                                                       #n//2 divides n by 2 without any remainder, i.e 6//2=3 and 7//2=3. So this line checks to see if i is equal to half of n without remainder. it will be yes once in the loop. not sure of its use
    ...
    if (epoch) % print_every_other == 0:        
        ...
        th, rh = plot_3D(in_data, rec_data)
        true_hits.append(th)
        recovered_hits.append(rh)
...
enc_input_size = (batch_size, 1, 128, 88, 100)

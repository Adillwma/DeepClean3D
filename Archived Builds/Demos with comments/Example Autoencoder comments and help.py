...
class Encoder(nn.Module):
    ...
    def __init__(self, encoded_space_dim,fc2_input_dim):            #encoded space dim is the dim after the encoder has been run. Fc2 is never used? Bad code
        super().__init__()
        ...
        #Output channels defines the number of seperate kernals that will be run across image, all which produce their own output arrays
        #The kernal size, is the size of the convolving layer. best visualised at the link above. Ours is set at 3 so it is a 3x3 matrix, can be 3x1 3x4 etc.
        ...
        self.encoder_cnn = nn.Sequential(           # this sequential thing just makes sure each of the following commands happens in sequence
            ...
            nn.BatchNorm2d(16),                            #BatchNorm minimises internal covariate shift (phenominon of each layers input distribution changing as the parameters of the layer above change during training)
            # This basically standardises it to a normal distribution center 0, varience v. It then applies mx + c to each of the normalised outputs before running through ReLU.
            # This is done to revent no gradient decent in sigmoid activation functions, and not sure about relu tbh. (maybe to give more chance for optimisation?)
            nn.ReLU(True),                                 #ReLU activation function - Activation function determinines if a neuron fires, i.e is the output of the node considered usefull. also allows for backprop. the arg 'True' makes the opperation carry out in-place(changes the values in the input array to the output values rather than making a new one), default would be false
        ...
        #using formulas on pytorch.org
        #layer1: [batchsize, 8, 14.5, 14.5] ----> [batchsize, 8, 14, 14] (as dont keep decimals)
        #layer2: [batchsize, 16, 7.5, 7.5] ----> [batchsize, 16, 7, 7] (as dont keep decimals)
        #layer3: [batchsize, 32, 3.5, 3.5] ----> [batchsize, 32, 3, 3] (as dont keep decimals)
        #by layer 3, we now have 32 sets of 3x3 arrays
        
        ###Flatten layer
        self.flatten = nn.Flatten(start_dim=1)    # Flatten reduces any dimensional array to 1D by lining them all up e.g. ((1,2),(3,4)) --> (1,2,3,4)
        # e.g. this takes our output from before, 32x3x3, and does 32*3*3=288
    ...
    def forward(self, x):           #This actually combines all the things before (x is the input data)
        x = self.encoder_cnn(x)                           #Runs convoloutional encoder on the input data of dinension - [mini-batch dim, channel dim, spacial dimension, spacial dimension] (so our data would have 1 channel dim, and 3 spacial dims)
        x = self.flatten(x)                               #Runs flatten  on output of conv encoder to get it in the right format for the linear encoder
...
class Decoder(nn.Module):
    ...
    def __init__(self, encoded_space_dim,fc2_input_dim):
        ...
        #The kernal size, 
        #Stride 
        #Output_Padding adds padding to the output (added it to input before so must reverse this)

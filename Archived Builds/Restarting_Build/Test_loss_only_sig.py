...
def plot_ae_outputs_den(encoder,decoder,n=10,noise_factor=0.3):       #Defines a function for plotting the output of the autoencoder. And also the input + clean training data? Function takes inputs, 'encoder' and 'decoder' which are expected to be classes (defining the encode and decode nets), 'n' which is the number of ?????Images in the batch????, and 'noise_factor' which is a multiplier for the magnitude of the added noise allowing it to be scaled in intensity.  
    ...
    for i in range(int(n/2)):                                                #Runs for loop where 'i' itterates over 'n' total values which range from 0 to n-1
      ...
      #plt.scatter(non_zero_img_indices[0], non_zero_img_indices[1], z_recon)
      ax.get_xaxis().set_visible(False)                                   #Hides the x axis from showing in the plot as we are plotting images not graphs (we may want to retain axis?)
      ax.get_yaxis().set_visible(False)                                   #Hides the y axis from showing in the plot as we are plotting images not graphs (we may want to retain axis?)
      if i == n//2:                                                       #n//2 divides n by 2 without any remainder, i.e 6//2=3 and 7//2=3. So this line checks to see if i is equal to half of n without remainder. it will be yes once in the loop. not sure of its use
    ...
    if rec_data.ndim != 1:
        ...
        # this is finding the numpy indices of the 
    print(np.shape(img.cpu().squeeze().numpy()))
    non_zero_img_indices = np.nonzero(img.cpu().squeeze().numpy())
    only_sig_img = np.zeros_like(img.cpu().squeeze().numpy())
    only_sig_img[non_zero_img_indices] = rec_img.cpu().squeeze().numpy()[non_zero_img_indices]
    
    # now we plot a 3d diagram so we can see better:
    # first make them all x,y,z:
    z_recon = only_sig_img[non_zero_img_indices]
    z_orig = img.cpu().squeeze().numpy()[non_zero_img_indices]

    fig = plt.figure()
    ax = plt.axes(projection='3d')
    ax.scatter(non_zero_img_indices[0], non_zero_img_indices[1], z_orig, color='red', label='Label points')
    ax.scatter(non_zero_img_indices[0], non_zero_img_indices[1], z_recon, color='blue', label='Recon points')
    ax.legend()
    plt.show()

    # here simply plot the reconstruction in 3d:
    x3d, y3d = np.meshgrid(np.arange(rec_img.cpu().squeeze().numpy().shape[1]), np.arange(rec_img.cpu().squeeze().numpy().shape[0]))
    coordinates = np.stack((x3d, y3d, rec_img.cpu().squeeze().numpy()), axis=-1)
    fig = plt.figure()
    ax = plt.axes(projection='3d')
    ax.scatter(coordinates[:,:,0], coordinates[:,:,1], coordinates[:,:,2], color='blue', label='Recon points')
    ax.legend()
    plt.show()
...
class sig_only(torch.nn.Module):
    def __init__(self, reduction='none'):
        super(sig_only, self).__init__()
...
class RatioLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        # find values for ratio
        hits = torch.nonzero(target)
        num_hits = hits.shape[0]
        all_pixels = target.numel()

        signals = tuple(torch.nonzero(target).t())
        ...
        if self.reduction == 'mean':
            ...
            raise ValueError("Invalid reduction type. Should be 'mean' or 'sum'.")
